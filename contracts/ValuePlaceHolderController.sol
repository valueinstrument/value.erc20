...
//Controller place holder - currently doesn't do much
...
contract ValuePlaceHolderController is TokenController, Owned {
    using SafeMath for uint256;

    MiniMeToken public v;

    function ValueDevController(address _owner, address _v) {
        owner = _owner;
        v = MiniMeToken(_v);
    }

    /// @notice The owner of this contract can change the controller of the V token
    ///  Please, be sure that the owner is a trusted agent or 0x0 address.
    /// @param _newController The address of the new controller
    function changeController(address _newController) public onlyOwner {
        v.changeController(_newController);
